      ...
      switch (true) {
        case element === 'Container': {
          ...
          const components = React.Children.map(child.props.children, (component) => {
            ...
            if (name === 'VideoComponent') return this.cloneElement(component, key);
            if (this.state.fullscreen && name !== 'VideoComponent') return null;
            return component
          });
          return React.cloneElement(child, props, components);
        }
        case element === 'VideoComponent':
          return this.cloneElement(child, key);
        case (this.state.fullscreen && element !== 'VideoComponent'):
          return null;
        default:
          return child
      }
